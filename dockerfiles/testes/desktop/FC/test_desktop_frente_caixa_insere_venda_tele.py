...
def test_desktop_insere_venda(env_vars, log_manager,timestampFormat):
    ...
    log_manager.add_log(
                                application_type=env_application_type,
                                level="DEBUG",
                                message=f"Cliente escolhido: '{client}'",
                                routine="Inserção de venda", 
                                error_details=''
                                )
    try:
        ...
        left, top, right, bottom = 349, 95, 1469, 120  # Coordenadas fornecidas "Campo de inserir produto [l=349,t=95,r=1469,b=120]"
        ...
        before_insert_screenshot_path = f"{screenshoot_path}\\before_insert_{timestamp}.png"
        ...
        left1, top1, right1, bottom1 = 1864, 218, 1894, 248  # Coordenadas fornecidas "Campo de frete [l=1864,t=218,r=1894,b=248]"

        # Definir a região específica da captura
        region = (left1, top1, right1, bottom1) 

        # 1. Capturar screenshot antes da inserção
        before_insert_screenshot_path1 = f"{screenshoot_path}\\before_insert_1_{timestamp}.png"
        ...
        left2, top2, right2, bottom2 = 1617, 259, 1920, 934  # Coordenadas fornecidas "Campo produto carrinho [l=1617,t=259,r=1920,b=934]"
        # Definir a região específica da captura
        ...
        after_insert_screenshot_path = f"{screenshoot_path}\\after_insert_1_{timestamp}.png"
        ...
        if difference < 1.5:  # Define um limite de tolerância de 3% (ajuste conforme necessário)
            log_manager.add_log(
                                ...
                                level="ERROR",
                                message="",
                                routine=f"Erro: Nenhuma mudança detectada! Diferença de apenas {difference:.2f}% Produto não inserido no campo produto", 
                                error_details=''
                                )        
        else:
            log_manager.add_log(
        ...
        # 1. Capturar screenshot antes da inserção
        after_insert_screenshot_path2 = f"{screenshoot_path}\\after_insert_2_{timestamp}.png"
        ...
        if difference2 < 1.5:  # Define um limite de tolerância de 3% (ajuste conforme necessário)
            log_manager.add_log(
                                application_type=env_application_type, 
                                level="ERROR",
                                message="",
            ...
            left3, top3, right3, bottom3 = 1623, 96, 1914, 156  # Coordenadas fornecidas "Campo Cliente [l=1688,t=96,r=1874,b=156]"
            ...
            # 1. Capturar screenshot antes da inserção
            after_insert_screenshot_path3 = f"{screenshoot_path}\\after_insert_3_{timestamp}.png"
            ...
            if difference3 < 1.5:
                log_manager.add_log(
                                    ...
                                    level="ERROR",
                                    message="",
                                    routine=f"Erro: Nenhuma mudança detectada! Diferença de apenas {difference3:.2f}% Cliente não vinculado a compra", 
                                    error_details=''
                                    )   
            else:
                ...
                log_manager.add_log(
                                    ...
                                    level="INFO",
                                    message="",
                                    routine=f"mudança detectada! Diferença{difference3:.2f}% Produto adicionado ao carrinho", 
                                    error_details=''
                                    )        
        ...
        # 1. Capturar screenshot antes da inserção
        after_insert_screenshot_path4 = f"{screenshoot_path}\\after_insert_4_{timestamp}.png"
        after_insert_screenshot4 = ImageGrab.grab(bbox=region2)
        after_insert_screenshot4.save(after_insert_screenshot_path4)

        difference4 = compare_images_advanced(after_insert_screenshot_path4, after_insert_screenshot_path2)

        if difference4 < 1.5:  # Define um limite de tolerância de 3% (ajuste conforme necessário)
            log_manager.add_log(
                                application_type=env_application_type, 
                                level="ERROR",
                                message="",
                                routine=f"Erro: Nenhuma mudança detectada! Diferença de apenas {difference4:.2f}% Venda não realizada", 
                                error_details=''
                                )        
        else:
            log_manager.add_log(
                                ...
                                level="INFO",
                                message="",
                                routine=f"mudança detectada! Diferença {difference4:.2f}% Venda realizada", 
                                error_details=''
                                )             
