...
def test_desktop_insere_venda(env_vars, log_manager,timestampFormat):
    ...
    try:
        ...
        after_insert_screenshot3 = ImageGrab.grab(bbox=region2)
        after_insert_screenshot3.save(after_insert_screenshot_path3)

        difference3 = compare_images_advanced(after_insert_screenshot_path3, after_insert_screenshot_path2)
        ...
        if difference3 < 1.5:  # Define um limite de tolerância de 3% (ajuste conforme necessário)
            log_manager.add_log(
                                ...
                                level="ERROR",
                                message="",
                                routine=f"Erro: Nenhuma mudança detectada! Diferença de apenas {difference3:.2f}% Venda não realizada", 
                                error_details=''
                                )        
        else:
            log_manager.add_log(
                                ...
                                level="INFO",
                                message="",
                                routine=f"mudança detectada! Diferença {difference3:.2f}% Venda realizada", 
                                error_details=''
                                )             
