import time
import pytest
from testes.testesWeb.ConciliacaoBancaria.test_conciliacaoBancaria_extratosBancarios_novaConciliacao import test_conciliacaoBancaria_extratosBancarios_novaConciliacao
from testes.testesWeb.ConciliacaoBancaria.test_conciliacaoBancaria_queries import test_conciliacaoBancaria_queries
from testes.testesWeb.ConciliacaoBancaria.teste_conciliacaoBancaria_arquivoOfx_pendentes import test_conciliacaoBancaria_arquivoOfx_pendentes

@pytest.mark.parametrize("insertOrEdit", ["Insert"])  # Default para Insert
def test_module_conciliacaoBancaria(init, request, insertOrEdit):
    starTime = time.time()
    ...
    try:
        ...
        if insertOrEdit == 'Insert':
            query = test_conciliacaoBancaria_queries(init)
            test_conciliacaoBancaria_extratosBancarios_novaConciliacao(init,query)
            test_conciliacaoBancaria_arquivoOfx_pendentes(init,query)
        ...
        Log_manager.add_log(
            ...
            level="ERROR",
            message="Erro na execução do módulo de Conciliação Bancaria",
