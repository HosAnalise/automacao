...
def test_contaPagar_insereConta_repeticao(init):
    starTime = time.time()
    ...
    try:
        ...
        ContasPagar.repeticaoContaPagar(init)
        ContasPagar.finalizaInsertContaPagar(init)   
        

    except (TimeoutException, NoSuchElementException, Exception) as e:
        Log_manager.add_log(application_type=env_application_type, level="ERROR", message=str(e), routine=f"{ContasPagar.rotina} - test_contaPagar_insereConta_repeticao", error_details=str(e))
        ...
        Log_manager.add_log(
            ...
            routine=f"{ContasPagar.rotina} - test_contaPagar_insereConta_repeticao",
            error_details=''
        )
