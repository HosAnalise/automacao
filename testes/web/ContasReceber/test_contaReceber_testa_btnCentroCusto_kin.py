...
@pytest.mark.parametrize("infoConta, devePassar", [
    (
        ContaReceber.Conta(
            detalhes=ContaReceber.DetalhesConta(P85_CENTRO_DE_CUSTO='3228') #qualquer um menos 2856
        ),
        True
    )
...
def test_contaReceber_botao_centroDeCusto(init, infoConta, devePassar):
    starTime = time.time()
    ...
    try:
        ...
        if ContaReceber.insereContaCompleta(init, infoConta):

            funcionou = ContaReceber.testaBtnCentroCusto(init)
            ...
            if devePassar:
                if funcionou is False:
                    Log_manager.add_log(
                        ...
                        routine=f"{ContaReceber.rotina} - test_contaReceber_botao_centroDeCusto",
                        error_details=""
                    )
        ...
        Log_manager.add_log(application_type=env_application_type, level="ERROR", message=str(e), routine=f"{ContaReceber.rotina} - test_contaReceber_botao_centroDeCusto", error_details=str(e))
