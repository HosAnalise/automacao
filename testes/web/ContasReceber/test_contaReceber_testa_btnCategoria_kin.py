...
@pytest.mark.parametrize("infoConta, devePassar", [
    (
        ContaReceber.Conta(
            P85_CATEGORIA_FINANCEIRA='33087'
        ),
        True
    )
...
def test_contaReceber_botao_categoria(init, infoConta, devePassar):
    starTime = time.time()
    ...
    try:
        ...
        if ContaReceber.preencheCamposConta(init, infoConta):
            funcionou = ContaReceber.testaBtnCategoria(init)
            ...
            if devePassar:
                if funcionou is False:
                    Log_manager.add_log(
                        ...
                        routine=f"{ContaReceber.rotina} - test_contaReceber_botao_categoria",
                        error_details=""
                    )
        ...
        Log_manager.add_log(application_type=env_application_type, level="ERROR", message=str(e), routine=f"{ContaReceber.rotina} - test_contaReceber_botao_categoria", error_details=str(e))
